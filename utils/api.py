...
# used to record rate limit related metrics
...
def post_api(path, caller_name=None, body=None):
    ...
    if "character" in data["data"]:
        state.set_current_character(data["data"]["character"])
...
def unequip(slot, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Unequipping item(s)")
...
def delete_item(item_code, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Deleting item(s)")
...
def deposit_gold(quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Depositing gold")
...
def deposit_item(item_code, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Depositing item(s)")
...
def withdraw_gold(quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Withdrawing gold")
...
def buy_npc_item(item_code, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Buying item(s) (from NPC)")
...
def sell_npc_item(item_code, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Selling item(s) (to NPC)")
...
def recycle(item_code, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Recycling item(s)")
...
def buy_exchange_item(order_id, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Buying item(s) (from exchange)")
...
def create_sell_order_exchange_item(item_code, price, quantity=1):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Creating sell order")
...
def get_character(name=None):
    ...
    func_name = inspect.currentframe().f_code.co_name
    logger.info(f'API: Getting information about {name}')
...
def list_item_sale_history(item_code, page=1, page_size=100, buyer=None, seller=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing exchange sell history")
    return get_api(
...
def list_item_listings(item_code, page=1, page_size=100, seller=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing active sell orders for item")
    return get_api(
...
def list_listings(page=1, page_size=100, seller=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing all active sell orders")
    return get_api(
        "grandexchange/orders",
        func_name,
        {
...
def get_item_listing(listing_id):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Getting active sell order")
...
def list_items(
    ...
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing item glossary")
...
def get_item(item_code):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Looking up item information from glossary")
...
def get_monsters_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for monsters")
...
def get_resources_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for resources")
...
def get_workshops_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for workshops")
...
def get_banks_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for banks")
...
def get_grand_exchanges_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for exchanges")
...
def get_tasks_masters_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for task masters")
...
def get_npcs_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking map for NPCs")
    return get_map(page=page, page_size=page_size, content_code=content_code, content_type="npc", _caller_name=func_name)

@register("Data")
def get_content_locations_on_map(page=1, page_size=100, content_code=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info(f"API: Checking map for {content_code}")
    return get_map(page=page, page_size=page_size, content_code=content_code, _caller_name=func_name)

@register("Data")
def get_location(x, y):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking location info")
...
def list_monsters(page=1, page_size=100, item_drop_code=None, max_level=None, min_level=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing monsters from glossary")
...
def list_resources(page=1, page_size=100, drop_code=None, skill_id=None, max_level=None, min_level=None):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing resources from glossary")
...
def get_resource(resource_code):
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Checking resource information from glossary")
...
def list_tasks(
    ...
    func_name = inspect.currentframe().f_code.co_name
    logger.info("API: Listing tasks from glossary")
...
def delete_character(character_name, im_sure=False):
    ...
    func_name = inspect.currentframe().f_code.co_name
    logger.info(f"API: deleting character {character_name}")
